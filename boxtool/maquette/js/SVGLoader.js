...
SVGLoader.prototype = Object.assign( Object.create( THREE.Loader.prototype ), {
	...
	load: function ( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			...
			const { paths, root } = scope.parse( text )
			onLoad( paths, root );
	...
	parse: function ( text ) {
		...
		function parseNode( node, style ) {
			...
			switch ( node.nodeName ) {
				...
				case 'svg':
					//root.base = node.viewBox.baseVal;
					break;
			...
			if ( path ) {
				if (node.nodeName === "line")
					root.lines.push(node);
				else
					root.nodes.push(node);

				path.label = node.id || style.id;
				node.id = node.id || style.id;
		...
		*/
		...
		var data = { paths: paths, root: root, xml: xml.documentElement };
	...
	getStrokeStyle: function ( width, color, lineJoin, lineCap, miterLimit ) {
	...
	pointsToStroke: function ( points, style, arcDivisions, minDistance ) {
	...
	pointsToStrokeWithBuffers: function () {
